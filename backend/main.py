...
def _process_tender_background(tender_id: int):
    ...
    with Session(database.engine) as session:
        ...
        if not tender:
            return
        try:
            # 1. Extract criteria for bidder evaluation
            criteria_list = extract_criteria_from_tender(tender.raw_text)
            for c in criteria_list:
                criterion = models.Criterion(
                    tender_id=tender_id,
                    criterion_type=c.get("criterion_type", "compliance"),
            ...
            # 2. Full structured analysis (Documents, Scope, Eligibility, Contacts)
