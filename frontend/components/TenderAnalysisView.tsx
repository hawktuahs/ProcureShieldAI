  ...
  return (
    <SectionCard title="Work Description" icon={FileText}>
      ...
      <InfoGrid items={[
        ...
        { label: "Total Quantity",    value: ov.total_quantity ? `${ov.total_quantity} ${ov.quantity_unit || ""}` : null },
      ]} />
      {ov.beneficiary && (
...
function ItemsTab({ items, filename, onReanalyze, tenderId }: {
  ...
  const totalQty = uniqueItems.reduce((acc, it) => {
    // Ensure quantity is a string before calling .replace
    const qtyStr = it.quantity !== null && it.quantity !== undefined ? String(it.quantity) : "0";
    const n = parseInt(qtyStr.replace(/,/g, ""), 10);
