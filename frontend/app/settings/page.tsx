...
export default function SettingsPage() {
  ...
  const handleSave = () => {
    // Key is intentionally session-only and not persisted to disk
    setSaved(true);
        ...
        <Card className="border border-slate-200">
          ...
          <CardContent className="px-5 pb-5 flex flex-col gap-5">
            <div className="flex items-start justify-between gap-4 p-4 bg-slate-50 border border-slate-200 rounded-lg">
              ...
              <Badge variant="outline" className="text-emerald-600 bg-emerald-50 border-emerald-200 shrink-0">Active</Badge>
            </div>
            ...
            <div className="p-4 bg-slate-50 border border-slate-200 rounded-lg">
              <div className="flex items-center justify-between mb-3">
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-0.5">Set <code className="bg-slate-100 px-1 py-0.5 rounded text-[11px]">LLM_PROVIDER=gemini</code> to switch. Key is session-only and never stored to disk.</p>
                </div>
              ...
              <div className="flex gap-2">
                ...
                <Button
                  onClick={handleSave}
                  disabled={!geminiKey}
                  className="bg-[#1e3a5f] hover:bg-blue-800 shrink-0"
                  size="sm"
                >
                  {saved ? <><CheckCircle2 className="w-3.5 h-3.5" /> Saved</> : <><Save className="w-3.5 h-3.5" /> Set Key</>}
                </Button>
              </div>
            </div>
          </CardContent>
